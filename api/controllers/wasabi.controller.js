...
class WasabiController {
    ...
    static async getDeliveryFile(req, res) {
        try {
            const { id } = req.params;
            const fileContent = await WasabiService.getDeliveryFile(id);

            if (fileContent) {
                // Définir les en-têtes de réponse pour spécifier le type de contenu et le nom du fichier
                res.setHeader('Content-Type', 'text/html');
                res.setHeader('Content-Disposition', `attachment; filename="delivery_map_${id}.html"`);
